...
<html lang="es">
	...
	<body>
		...
		<div class="container-fluid margen-superior-registros-consultor">
			...
			<div class="row">
				<div class="col-lg-12">
					<a href="buscar_inmueble_consultor.php" class="btn btn-primary btn-sm btnBuscarConsultor">B&uacute;squeda de inmuebles</a>
				</div>
			</div>

			<?php
				// ================= VERIFICACIÓN DE LA EXISTENCIA DE LA VARIABLE $_GET["paginaInmueblesConsultor"] EN LA URL =================
				if (isset($_GET["paginaInmueblesConsultor"])) {
					// ===== Calculando el 'desde' y el 'hasta' para la paginación =====:
					$inmueblesPorPagina = 5; // Cantidad de usuarios a mostrar por página.
					$inicio = ($_GET["paginaInmueblesConsultor"]-1) * $inmueblesPorPagina;
		    		...
		    		$_GET["paginaInmueblesConsultor"] = 1;
			  ...
			  <ul class='pagination pagination-sm justify-content-end'>
			    <?php
			    	if (isset($_GET["paginaInmueblesConsultor"])) {
				    	...
				    	if ($_GET['paginaInmueblesConsultor'] <= 1) {
				    		echo "<li class='page-item disabled'><a href='registros_consultor.php?paginaInmueblesConsultor=".($_GET['paginaInmueblesConsultor']-1)."' class='page-link'>&laquo; Anterior</a></li>";
				    	}else{
				    		echo "<li class='page-item'><a href='registros_consultor.php?paginaInmueblesConsultor=".($_GET['paginaInmueblesConsultor']-1)."' class='page-link'>&laquo; Anterior</a></li>";
				    	}

				    	// Botones 1,2,3,4,5... generados automáticamente.
				    	for ($i=0; $i < $totalPaginas; $i++) { 
				    		if (($i+1) == $_GET['paginaInmueblesConsultor']) { // Verificación para aplicar la clase "Active de bootstrap".
				    			echo "<li class='page-item active'><a href='registros_consultor.php?paginaInmueblesConsultor=".($i+1)."' class='page-link'>".($i+1)."</a></li>";
				    		}else{
				    			echo "<li class='page-item'><a href='registros_consultor.php?paginaInmueblesConsultor=".($i+1)."' class='page-link'>".($i+1)."</a></li>";
				    		}
				    	}
				    	...
				    	if (($_GET['paginaInmueblesConsultor'] >= $totalPaginas) || (mysqli_num_rows($registros) == 1)) {
				    		echo "<li class='page-item disabled'><a href='registros_consultor.php?paginaInmueblesConsultor=".($_GET['paginaInmueblesConsultor']+1)."' class='page-link'>Siguiente &raquo;</a></li>";
				    	}else{
				    		echo "<li class='page-item'><a href='registros_consultor.php?paginaInmueblesConsultor=".($_GET['paginaInmueblesConsultor']+1)."' class='page-link'>Siguiente &raquo;</a></li>";
				    	}
			    	}
			    ?>
